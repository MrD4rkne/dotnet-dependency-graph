...
use dependency_parser::models::parse_dependency_graph_spec;

#[test]
fn dgspec_for_project_and_libraries_dependencies_deserializing_then_serializing_produces_equivalent_json() -> std::io::Result<()> {
	// Arrange
	...
	assert!(!dgspec_files.is_empty(), "no .nuget.dgspec.json files found in {:?}", sln_dir);
	...
	for path_to_dgspec in dgspec_files {
        ...
        // Assert
		assert_eq!(original_content, serialized_deserialized_content, "roundtrip mismatch for file {:?}", path_to_dgspec);
	}
...
fn clean_dotnet_sln(sln_path: &std::path::Path) -> std::io::Result<()>{
	...
	match status.success() {
		true => Ok(()),
		false => Err(std::io::Error::new(std::io::ErrorKind::Other, format!("Dotnet clean failed with exit status: {}", status)))
	}
}
...
fn add_dgspecs_from_dir(dir: &std::path::Path, out: &mut Vec<PathBuf>) -> std::io::Result<()> {
	for entry in fs::read_dir(dir)? {
		...
		if path_to_file.is_dir() {
			...
			if name.ends_with(".nuget.dgspec.json") {
